...
import { Gs1BeluUploadClient } from "../gs1BeluUploadClient.js";
import type { Gs1BeluCredentials } from "../credentials.js";
import { FakeFetch } from "./fakeFetch.js";

export const FAKE_BASE_URL = "https://fake-api.test/myproductmanager/upload/v17";
 ...
 * middleware pipeline as `Gs1BeluUploadClient`'s derived-config constructor form, but wired to fake
 * HTTP transports (API + token endpoint) instead of the real network. Consumers never see this — it
 * exists only so tests can assert on observable HTTP behavior through the real classes.
 */
export function createTestHarness(options: TestHarnessOptions = {}) {
  ...
  const client = new Gs1BeluUploadClient({ requestAdapter: adapter });
