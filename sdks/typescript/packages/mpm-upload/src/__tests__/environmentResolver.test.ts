...
test("uat derives the uat hosts and audience", () => {
  ...
  assert.equal(Gs1BeluEnvironmentResolver.baseUrl("uat", "v17"), "https://api-uat.gs1belu.org/myproductmanager/upload/v17");
});
...
test("baseUrl honors a non-default apiVersion", () => {
  assert.equal(Gs1BeluEnvironmentResolver.baseUrl("prod", "v18"), "https://api.gs1belu.org/myproductmanager/upload/v18");
});
