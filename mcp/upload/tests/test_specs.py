"""Pins the packaged-resource spec-resolution path (issue #70's regression test,
carried through the #82 split): a future refactor that breaks bundling must fail
loudly here, instead of silently falling back to the dev path on a machine where that
happens to still work by accident.
"""
...
from gs1belu_mpm_upload import specs

_REPO_ROOT = Path(__file__).resolve().parents[3]
_PACKAGED_SPECS = Path(__file__).resolve().parents[1] / "src" / "gs1belu_mpm_upload" / "_specs"


@pytest.fixture
def bundled_packaged_specs():
    """Reproduces what `hatch_build.py`'s `CustomBuildHook` does at build time — copies
    the effective spec into the packaged `_specs/` resource home — so this test can pin
    the packaged-resource branch without requiring a real `uv build`."""
    src = _REPO_ROOT / "schemas" / "upload" / "v17.effective.yaml"
    dst = _PACKAGED_SPECS / "upload" / "v17.effective.yaml"
