"""Pins `upsert_and_await_validation`'s contract (#46, user stories 3-5, carried
verbatim through the #82 split): submits the item, polls until `metaData.status`
leaves `pendingValidation`, returns the settled verdict, and honors a bounded overall
...
async def test_returns_the_settled_verdict_once_validation_leaves_pending(server, transport):
    transport.queue("POST", "/tradeitems", httpx.Response(201, headers={"Location": "/tradeitems/05412345678900"}))
    transport.queue("GET", "/tradeitems/{gtin}", trade_item_status(TRADE_ITEM["gtin"], "pendingValidation"))
    transport.queue("GET", "/tradeitems/{gtin}", trade_item_status(TRADE_ITEM["gtin"], "pendingValidation"))
    transport.queue(
        "GET",
        "/tradeitems/{gtin}",
    ...
    get_calls = [r for r in transport.requests if r.method == "GET"]
    assert len(get_calls) == 3, "must stop polling as soon as status leaves pendingValidation"


async def test_incomplete_verdict_is_returned_not_raised(server, transport):
    transport.queue("POST", "/tradeitems", httpx.Response(201))
    transport.queue(
        "GET",
        "/tradeitems/{gtin}",
        trade_item_status(
...
async def test_bounded_timeout_stops_an_unbounded_spin(server, transport):
