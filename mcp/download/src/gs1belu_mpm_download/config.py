...
`GS1BELU_DOWNLOAD_*`, never `GS1BELU_UPLOAD_*` — a data-recipient integrator
configures exactly the credentials their role holds. `parse_environment()` and
`CredentialSet` are single-sourced in `_shared/` so this server and its Upload
...
CREDENTIAL_PREFIX = "GS1BELU_DOWNLOAD"
