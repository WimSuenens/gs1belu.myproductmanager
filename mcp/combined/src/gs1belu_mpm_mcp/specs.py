"""Loads the effective specs (`<api>/<version>.effective.yaml`) that `just gen`
produces from the pristine vendor originals + overlay — the same artifact Kiota reads
...
3. `schemas/<api>/<version>.effective.yaml` relative to a repo checkout — the
...
def _packaged_spec_text(api: str, version: str) -> str | None:
    try:
        resource = files("gs1belu_mpm_mcp").joinpath("_specs", api, f"{version}.effective.yaml")
