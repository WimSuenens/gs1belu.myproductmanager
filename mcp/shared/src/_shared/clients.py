"""Builds the one authenticated `httpx.AsyncClient` each server (and its hand-written
tools) uses — base URL derived from `environment`, `Gs1BeluAuth` for both credential
layers, `RateLimiter` for the 10 req/s cap, and the Sunset-observing and structured-
error response hooks. `transport` is the one testability seam: omit it for real network
use, inject an `httpx.MockTransport` in tests so both the upstream API host and the
token host route through the same fake — it is never exposed on the server's public
config.
"""
