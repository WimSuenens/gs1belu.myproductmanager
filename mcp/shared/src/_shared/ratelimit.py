...
Attached as an httpx `event_hooks={"request": [...]}` hook on each server's own shared
client (#77 — one `RateLimiter` per process now, never shared across the two servers),
so it paces every outgoing request through that client uniformly.
"""
