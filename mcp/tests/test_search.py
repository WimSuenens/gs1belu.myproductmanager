"""Pins `search_trade_items`'s context-safety contract (#46, user stories 7-9): one
bounded page plus an opaque `nextCursor`, never server-side auto-pagination — even
against a canned multi-page HAL response that has further pages available.
"""
...
async def test_returns_one_page_plus_next_cursor_without_auto_pagination(server, download_transport):
    download_transport.queue("GET", "/tradeitems", httpx.Response(200, json=HAL_PAGE_ONE))
    ...
    api_requests = [r for r in download_transport.requests if r.url.host != download_transport.token_host]
    assert len(api_requests) == 1, "must not auto-follow _links.next itself"


async def test_next_cursor_is_null_on_the_last_page(server, download_transport):
    download_transport.queue("GET", "/tradeitems", httpx.Response(200, json=HAL_LAST_PAGE))
...
async def test_a_follow_up_call_with_the_cursor_advances(server, download_transport):
    download_transport.queue("GET", "/tradeitems", httpx.Response(200, json=HAL_PAGE_ONE))
    download_transport.queue("GET", "/tradeitems", httpx.Response(200, json=HAL_LAST_PAGE))
    ...
    second_request = download_transport.requests[-1]
